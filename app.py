...
def highlight_ai(s):
    for phrase in AI_PHRASES:
        s = re.compile(re.escape(phrase), re.IGNORECASE).sub(
            lambda m: f'<mark style="background:#fef3c7;border-radius:3px;padding:0 2px;font-weight:500;color:#78350f">{m.group(0)}</mark>', s)
    return s
...
<div class="precheck">
  ...
  <div class="timeline-row"><div class="timeline-num">1</div><div><div class="timeline-title">Analyze article</div><div class="timeline-sub">Editor comments, plagiarism, AI content.</div></div></div>
...
def render_report(sub):
    ...
    plag_row = (brow("ded-row", f'Plagiarism {ded["plag_pct"]}% ({ded["plag_brackets"]} × 5 pts)', f'−{ded["plag_deduction"]} pts')
                if ded["plag_deduction"] > 0 else brow("ok-row", f'Plagiarism {ded["plag_pct"]}% — under 20%', "no deduction"))
    ai_row   = (brow("ded-row", f'AI content {ded["ai_pct"]}% ({ded["ai_brackets"]} × 5 pts)', f'−{ded["ai_deduction"]} pts')
    ...
    with pc1:
        ...
        thresh = (f'<span class="detect-thresh" style="background:#fee2e2;color:#991b1b">{pp}% — over 20% — 5 pts deducted</span>'
                  if over else f'<span class="detect-thresh" style="background:#d1fae5;color:#065f46">{pp}% — under 20% — no deduction</span>')
        snip_html = ""
    ...
    with pc2:
        ...
        a_thresh = (f'<span class="detect-thresh" style="background:#fee2e2;color:#991b1b">{ap}% — over 20% — 5 pts deducted</span>'
                    if ai_over else f'<span class="detect-thresh" style="background:#d1fae5;color:#065f46">{ap}% — under 20% — no deduction</span>')
        ai_snip = ""
